...
POST /predict
    Classify a single ad-hoc message and save the prediction.

POST /predict/batch
    Run the decision engine over every ingested message.

GET /predict/{message_id}
    Fetch a stored prediction.
"""
...
def _save_and_build_response(
    db: Session,
    message_id: str,
    result: DecisionResult,
) -> PredictionOut:
    """
    Save a DecisionResult to the database and convert it
...
def predict_single(
    ...
    The message is first created in the database,
    then passed to the decision engine.
    """
    ...
    # Make sure the SQLAlchemy relationship is populated.
    message.sender = sender

    result = decide(message, db=db)

    response = _save_and_build_response(
        db,
        message.id,
        result,
    )
...
def predict_batch(
    ...
    """
    Run predictions for every message currently stored
    in the database.
    """
    ...
    if not messages:
        raise HTTPException(
            ...
            detail=(
                "No messages found. "
                "Upload a dataset first via POST /upload."
            ),
        )
...
def get_prediction(
    ...
    """
    Return the stored prediction for a message.
    """
    ...
    prediction = get_prediction_by_message_id(
        db,
        message_id,
    )
    ...
    if prediction is None:
        raise HTTPException(
            status_code=404,
            detail=(
                "No prediction yet for this message. "

...
export default function UploadPage() {
  ...
  const { markDatasetUploaded } = useDatasetSession();
  ...
  async function handleUpload(file: File) {
    ...
    try {
      // Upload the CSV to the backend
      const uploadResult = await uploadDataset(file);

      // Show upload result
      setResult(uploadResult);

      // Load the newly ingested messages
      const ingested = await listMessages(200);
      setMessages(ingested);

      // IMPORTANT:
      // Tell the frontend that a dataset has been uploaded

...
import { AlertTriangle, Bell, Inbox, ShieldOff, Upload } from "lucide-react";
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        {!loading && !error && data && data.total_messages === 0 && (
          <div className="flex flex-col items-center gap-3 rounded-card border border-border bg-surface py-16 text-center">
            ...
            <p className="max-w-sm text-sm text-text-muted">
              Upload a dataset to see live triage stats here.
            </p>
            <Link
              href="/upload"
              className="mt-2 rounded-full bg-notify px-4 py-2 text-sm font-medium text-ink transition-opacity hover:opacity-90"
            >
              Upload dataset
            </Link>
          </div>
        ...
        {!loading && !error && data && data.total_messages > 0 && (
          <div className="flex flex-col gap-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <StatCard label="Total messages" value={data.total_messages} icon={Inbox} />
              <StatCard
                label="Notified"
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              ...
              <StatCard
                label="Avg. scam risk"
                value={`${Math.round(data.avg_scam_probability * 100)}%`}
                accentClassName="text-mute"
              />
              <StatCard
                label="Avg. spam risk"
                value={`${Math.round(data.avg_spam_probability * 100)}%`}
                accentClassName="text-digest"
              />
            </div>
